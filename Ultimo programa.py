...
class TelaApp:
    ...
    def show_resultado(self, tela=None):
        ...
        if tela:
            mensaje = f"Tela encontrada: {tela}"
        else:
            mensaje = "No se encontró ninguna tela que cumpla con esas características."
        
        respuesta_label = tk.Label(self.root, text=mensaje, font=("Arial", 24), bg="blue", fg="white")
        respuesta_label.place(x=250, y=300)
        ...
        inicio_btn.place(x=100, y=500)

        otra_btn = tk.Button(self.root, text="Escribir otra respuesta", command=self.show_sin_respuesta, width=20)
        otra_btn.place(x=500, y=500)

    def show_sin_respuesta(self):
        self.clear_window()
        self.show_image(imagenes["sin_respuesta"])

        # Cuadro de texto para ingresar nueva tela y característica
        nombre_label = tk.Label(self.root, text="Nombre de la tela:", font=("Arial", 14))
        nombre_label.place(x=200, y=300)
        nombre_entry = tk.Entry(self.root, width=30, font=("Arial", 14))
        nombre_entry.place(x=350, y=300)

        caracteristica_label = tk.Label(self.root, text="Característica:", font=("Arial", 14))
        caracteristica_label.place(x=200, y=350)
        caracteristica_entry = tk.Entry(self.root, width=30, font=("Arial", 14))
        caracteristica_entry.place(x=350, y=350)

        guardar_btn = tk.Button(self.root, text="Guardar", width=10, command=lambda: self.guardar_nueva_tela(nombre_entry.get(), caracteristica_entry.get()))
        guardar_btn.place(x=350, y=450)

    def guardar_nueva_tela(self, nombre, caracteristica):
        if nombre and caracteristica:
            ...
            messagebox.showinfo("Guardado", f"Tela '{nombre}' con característica '{caracteristica}' guardada.")
            self.show_inicio()
        else:
